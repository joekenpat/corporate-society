              ...
              <div class="card-body">
                ...
                <div class="row">
                  <div class="col-xs-12 col-md-6">
                    ...
                    <select class="form-control form-control-lg @error('bank_code') form-error @enderror"
                      name="bank_code">
                      <option value="">Select Package</option>
                      @if(count($bankList))
                      @foreach ($bankList as $bankListItem)
                      <option value="{{$bankListItem->code}}">{{$bankListItem->name}}</option>
                      @endforeach
                      @endif
                    </select>
                    @error('bank_code')
                  ...
                  <div class="col-xs-12 col-md-6">
                    ...
                    <input class="form-control form-control-sm @error('account_number') form-error @enderror"
                      type="text" name="account_number" value="{{ old('account_number')?:($withdrawalBank->account_number??"") }}" placeholder="Account Number">
                  ...
                  <div class="col-xs-12 col-md-6">
                    ...
                    <input class="form-control form-control-sm @error('account_name') form-error @enderror" type="text"
                      name="account_name" value="{{ old('account_name')?:($withdrawalBank->account_name??"") }}" placeholder="Account Name">
                    @error('account_name')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                  </div>
